...
FORM HANDLE_UCOMM_0120 USING U_OKCODE.
  CASE U_OKCODE.
    ...
    WHEN 'CREATE_PACKAGE'.
      PERFORM PROCESS_CREATE_PACKAGE_DETAIL CHANGING GV_PACKAGE_ID.
      CLEAR: U_OKCODE.
...
*&
*&---------------------------------------------------------------------*
FORM HANDLE_UCOMM_0125 USING U_OKCODE.
  CASE U_OKCODE.
    WHEN 'CANCLE_125'.
      CLEAR: U_OKCODE.
      LEAVE TO SCREEN 0.
...
FORM PROCESS_PACKAGE_SAVE_EVENT.
  CASE GV_PACKAGE_SCREEN_MODE.
    ...
    WHEN GC_PACKAGE_MODE_CREATE.
      ...
      MESSAGE S016(Z03S24999_DOMUS_MSGS) WITH 'Package'.
...
*&---------------------------------------------------------------------*
FORM PROCESS_BACK_TO_PACKAGE_LIST.
  IF GV_PACKAGE_SCREEN_MODE = GC_PACKAGE_MODE_CREATE.
    PERFORM WARNING_EXIT_UNSAVED_PACKAGE.
  ELSE.
...
ENDFORM.
*&---------------------------------------------------------------------*
*& Form PROCESS_CREATE_PACKAGE_DETAIL
*&---------------------------------------------------------------------*
*& text
*&---------------------------------------------------------------------*
*& -->  CH_PACKAGE_ID
*& <--  CH_PACKAGE_ID
*&---------------------------------------------------------------------*
FORM PROCESS_CREATE_PACKAGE_DETAIL CHANGING CH_PACKAGE_ID.
  CLEAR: GV_PACKAGE_TOTAL_PRICE,
         GV_PCKPRV_TOTAL_PRICE,
         GV_PCKSER_TOTAL_PRICE,
         GS_PACKAGE_DETAIL_BEFORE_MOD,
         GS_PACKAGE_DETAIL,
         GT_PCKPRV_BEFORE_MOD,
         GT_PCKPRV,
         GT_PCKSER_BEFORE_MOD,
         GT_PCKSER,
         GT_PCKIMG_BEFORE_MOD,
         GT_PCKIMG,
         GT_PCKPRV_DELETED,
         GT_PCKSER_DELETED,
         GT_PCKIMG_DELETED,
  ...
  PERFORM CREATE_UUID_C36_STATIC CHANGING CH_PACKAGE_ID.

  IF PCKDES_CONTAINER IS INITIAL OR PCKDES_EDITOR IS INITIAL.

    CREATE OBJECT: PCKDES_CONTAINER EXPORTING CONTAINER_NAME = 'CUSTOM_CONTROL_TEXT_0129',
                   PCKDES_EDITOR    EXPORTING PARENT = PCKDES_CONTAINER
                                              MAX_NUMBER_CHARS = 1333.
  ENDIF.

* Set Screen Mode to View only
  GV_PACKAGE_SCREEN_MODE = GC_PACKAGE_MODE_CREATE.
* Change Screen from 0120 to 0129
  PACKAGE_SCREEN_MODE = '0129'.

  MESSAGE S000(Z03S24999_DOMUS_MSGS) WITH 'You can create new Package now.'.
...
FORM SHOW_PACKAGE_DESCRIPTION USING U_PCKDES TYPE Y03S24999_PACKGE-DESCRIPTION.
  ...
  IF SY-SUBRC = 0.
    PCKDES_TEXT_TAB_BEFORE_MOD = PCKDES_TEXT_TAB.
...
FORM PROCESS_PREPARE_0126_DATA USING U_PRODUCT_ID.
  ...
  IF SY-SUBRC <> 0.
    ...
    ENDIF.
    IF O_PROVRT_0126_HANDLER IS NOT INITIAL.
      CLEAR O_PROVRT_0126_HANDLER.
    ENDIF.
...
FORM CHANGE_PACKAGE_BASIC_DETAIL .
  ...
  CLEAR: PCKDES_TEXT_TAB.
  IF PCKDES_CONTAINER IS NOT INITIAL.
    PCKDES_EDITOR->GET_TEXT_AS_STREAM( IMPORTING TEXT = PCKDES_TEXT_TAB ).
  ENDIF.

  CLEAR: GS_PACKAGE_DETAIL-DESCRIPTION.
  CONCATENATE LINES OF PCKDES_TEXT_TAB INTO GS_PACKAGE_DETAIL-DESCRIPTION.

  MOVE-CORRESPONDING GS_PACKAGE_DETAIL TO LS_PACKGE.

  IF GV_PACKAGE_SCREEN_MODE = GC_PACKAGE_MODE_CHANGE.
  ...
  ELSEIF GV_PACKAGE_SCREEN_MODE = GC_PACKAGE_MODE_CREATE.
    LS_PACKGE-ID = GV_PACKAGE_ID.

    LS_PACKGE-CREATED_BY = SY-UNAME.
    LS_PACKGE-CREATED_AT = SY-UZEIT + ( 3600 * 5 ).
    LS_PACKGE-CREATED_ON = SY-DATUM.

    INSERT Y03S24999_PACKGE FROM LS_PACKGE.

    IF SY-SUBRC <> 0.
      MESSAGE E015(Z03S24999_DOMUS_MSGS) WITH 'Package basic information'.
    ENDIF.
...
FORM MODIFY_PCKPRV_TABLE .
  ...
  ELSEIF GV_PACKAGE_SCREEN_MODE = GC_PACKAGE_MODE_CREATE.
    LOOP AT LT_PI INTO DATA(LS_PIC).
      LS_PIC-CREATED_BY = SY-UNAME.
      LS_PIC-CREATED_AT = SY-UZEIT + ( 3600 * 5 ).
      LS_PIC-CREATED_ON = SY-DATUM.

      INSERT Y03S24999_PCKPRV FROM LS_PIC.

      IF SY-SUBRC <> 0.
        MESSAGE E015(Z03S24999_DOMUS_MSGS) WITH 'Package Product Variant details'.
      ENDIF.
    ENDLOOP.
...
FORM MODIFY_PCKSER_TABLE .
  ...
  ELSEIF GV_PACKAGE_SCREEN_MODE = GC_PACKAGE_MODE_CREATE.
    LOOP AT LT_PI INTO DATA(LS_PIC).
      ...
      INSERT Y03S24999_PCKSER FROM LS_PIC.

      IF SY-SUBRC <> 0.
        MESSAGE E015(Z03S24999_DOMUS_MSGS) WITH 'Package Service details'.
      ENDIF.
    ENDLOOP.
...
FORM MODIFY_PCKIMG_TABLE .
  ...
  ELSEIF GV_PACKAGE_SCREEN_MODE = GC_PACKAGE_MODE_CREATE.
    LOOP AT LT_PI INTO DATA(LS_PIC).
      ...
      INSERT Y03S24999_PCKIMG FROM LS_PIC.

      IF SY-SUBRC <> 0.
        MESSAGE E015(Z03S24999_DOMUS_MSGS) WITH 'Package Image details'.
      ENDIF.
    ENDLOOP.
...
ENDFORM.
*&---------------------------------------------------------------------*
*& Form WARNING_EXIT_UNSAVED_PACKAGE
*&---------------------------------------------------------------------*
*& text
*&---------------------------------------------------------------------*
*& -->  p1        text
...
FORM WARNING_EXIT_UNSAVED_PACKAGE .
  ...
  CALL FUNCTION 'POPUP_TO_CONFIRM'
    EXPORTING
      TEXT_QUESTION         = 'Do you want to Exit without saving?'
      TEXT_BUTTON_1         = 'Yes'(001)
      TEXT_BUTTON_2         = 'No'(002)
      DISPLAY_CANCEL_BUTTON = ''
    IMPORTING
...
FORM WARNING_PACKAGE_CHANGES_EXIST .
  ...
  IF GS_PACKAGE_DETAIL_BEFORE_MOD-NAME <> GS_PACKAGE_DETAIL-NAME OR
     PCKDES_TEXT_TAB_BEFORE_MOD <> PCKDES_TEXT_TAB OR
...
*      IS_VARIANT                    = IM_S_VARIANT
  ...
  IF O_PROVRT_0126_HANDLER IS INITIAL.
    O_PROVRT_0126_HANDLER = NEW CL_PACKAGE_PROVRT_ALV_HANDLER( ).
    SET HANDLER O_PROVRT_0126_HANDLER->HOTSPOT_CLICK FOR O_PROVRT_0126_ALV_TABLE.
  ENDIF.
...
* Define Table Structure / Define fields catalog
  PERFORM PREPARE_PRO_0125_FIELD_CATALOG
    CHANGING LT_FIELD_CAT.

* Prepare Layout
  PERFORM PREPARE_PROATV_0125_LAYOUT
    CHANGING LS_LAYOUT.
** Prepare Variant
*  PERFORM PREPARE_VARIANT
*    CHANGING LS_VARIANT.
...
* Show ALV
  PERFORM DISPLAY_PROATV_0125_ALV_TABLE
    CHANGING LS_LAYOUT
*            LS_VARIANT
             LT_FIELD_CAT.
ENDFORM.
*&---------------------------------------------------------------------*
*& Form ADD_PROATV_0125_FCAT
*&---------------------------------------------------------------------*
FORM ADD_PROATV_0125_FCAT USING U_FIELDNAME
                    U_SCRTEXT_M
                    U_OUTPUTLEN
                    U_KEY
                    U_HOTSPOT
...
ENDFORM.
*&---------------------------------------------------------------------*
*& Form PREPARE_PROATV_0125_FIELD_CATALOG
*&---------------------------------------------------------------------*
FORM PREPARE_PRO_0125_FIELD_CATALOG
  CHANGING CH_T_FIELD_CAT TYPE LVC_T_FCAT.

***** Full form:
  PERFORM: ADD_PROATV_0125_FCAT USING 'ATTRIBUTE_NAME' 'Attribute' 12 ''  'X' 'C500'  CHANGING CH_T_FIELD_CAT,
           ADD_PROATV_0125_FCAT USING 'VALUE'          'Value'     12 ''  ''  ''      CHANGING CH_T_FIELD_CAT.
ENDFORM.
*&---------------------------------------------------------------------*
*& Form DISPLAY_PROATV_0125_ALV_TABLE
*&---------------------------------------------------------------------*
FORM DISPLAY_PROATV_0125_ALV_TABLE
  USING    U_S_LAYOUT    TYPE LVC_S_LAYO
  CHANGING CH_T_FIELD_CAT TYPE LVC_T_FCAT.
*          IM_S_VARIANT   TYPE DISVARIANT
  ...
  IF O_PROATV_0125_CONTAINER IS INITIAL.
    O_PROATV_0125_CONTAINER = NEW CL_GUI_CUSTOM_CONTAINER( CONTAINER_NAME = 'CUSTOM_CONTROL_ALV_0125' ).
  ENDIF.

  IF O_PROATV_0125_ALV_TABLE IS INITIAL.
    O_PROATV_0125_ALV_TABLE = NEW CL_GUI_ALV_GRID( I_PARENT = O_PROATV_0125_CONTAINER ).
  ENDIF.

  O_PROATV_0125_ALV_TABLE->SET_TABLE_FOR_FIRST_DISPLAY(
    EXPORTING
      IS_LAYOUT                     = U_S_LAYOUT      " Layout
*      I_SAVE                        = 'A'
*      IS_VARIANT                    = IM_S_VARIANT
    CHANGING
      IT_OUTTAB                     = IT_PROATV_0125     " Output Table
      IT_FIELDCATALOG               = CH_T_FIELD_CAT   " Field Catalog
    EXCEPTIONS
      INVALID_PARAMETER_COMBINATION = 1                " Wrong Parameter
      PROGRAM_ERROR                 = 2                " Program Errors
...
ENDFORM.
*&---------------------------------------------------------------------*
*& Form PREPARE_PROATV_0125_LAYOUT
*&---------------------------------------------------------------------*
FORM PREPARE_PROATV_0125_LAYOUT CHANGING CH_S_LAYOUT TYPE LVC_S_LAYO.
...
ENDFORM.
*&---------------------------------------------------------------------*
*& Form PREPARE_PACKAGE_PROATV_0125
*&---------------------------------------------------------------------*
*& text
*&---------------------------------------------------------------------*
*&      --> U_PROVRT_ID
*&---------------------------------------------------------------------*
FORM PREPARE_PACKAGE_PROATV_0125  USING  U_PROVRT_ID.
  SELECT PROATV~VALUE, PROATR~ATTRIBUTE_NAME
    ...
    INTO CORRESPONDING FIELDS OF TABLE @IT_PROATV_0125.

  IF SY-SUBRC <> 0.
    CLEAR IT_PROATV_0125[].

    IF O_PROATV_0125_CONTAINER IS NOT INITIAL.
      CALL METHOD O_PROATV_0125_CONTAINER->FREE.
      CLEAR O_PROATV_0125_CONTAINER.
    ENDIF.
    IF O_PROATV_0125_ALV_TABLE IS NOT INITIAL.
      CLEAR O_PROATV_0125_ALV_TABLE.
    ENDIF.
