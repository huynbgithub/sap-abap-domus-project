...
FORM SET_C_CODE_INITIAL_VALUES.
  IF C_CODE[] IS INITIAL.
    C_CODE-SIGN   = 'I'.
    C_CODE-OPTION = 'BT'.
    C_CODE-LOW    = 'CT00000000'.
    C_CODE-HIGH   = 'CT99999999'.
    APPEND C_CODE.
  ENDIF.
ENDFORM.
...
FORM HANDLE_UCOMM_0100 USING U_OKCODE.
  CASE U_OKCODE.
    WHEN 'VIEW_ALL'.
      PERFORM PROCESS_CONTRACT_LIST_0100.
      CLEAR: U_OKCODE.
...
ENDFORM.
*&---------------------------------------------------------------------*
*& Form PROCESS_CONTRACT_LIST_0100
*&---------------------------------------------------------------------*
...
FORM GET_CONTRACT_DATA CHANGING CH_V_SUCCESS TYPE ABAP_BOOL.
  ...
  SELECT C~* FROM Y03S24999_CNTRCT AS C
    WHERE C~CONTRACT_CODE IN @C_CODE
      AND C~CUSTOMER = @GV_USERNAME
      AND IS_DELETED <> @ABAP_TRUE
    ORDER BY C~CREATED_ON DESCENDING, C~CREATED_AT DESCENDING, C~CONTRACT_CODE DESCENDING
...
FORM DISPLAY_CONTRACT_ALV_TABLE
  ...
  IF O_CONTRACT_CONTAINER IS INITIAL.
    O_CONTRACT_CONTAINER = NEW CL_GUI_CUSTOM_CONTAINER( CONTAINER_NAME = 'CUSTOM_CONTROL_ALV_0100' ).
  ENDIF.
...
* Prepare CONTRACT Detail to display on Screen 0200
...
FORM SHOW_CONTRACT_DESCRIPTION USING U_CTRDES TYPE Y03S24999_CNTRCT-DESCRIPTION.
  IF CTRDES_CONTAINER01 IS INITIAL OR CTRDES_EDITOR01 IS INITIAL.
    ...
    CREATE OBJECT: CTRDES_CONTAINER01 EXPORTING CONTAINER_NAME = 'CUSTOM_CONTROL_TEXT_0200',
...
FORM HANDLE_UCOMM_0200 USING U_OKCODE.
  CASE U_OKCODE.
    ...
    WHEN 'CANCEL'.
      PERFORM CANCEL_CONTRACT.
      CLEAR: U_OKCODE.

    WHEN 'SIGN'.
      PERFORM SIGN_CONTRACT.
      CLEAR: U_OKCODE.
...
*&---------------------------------------------------------------------*
FORM CANCEL_CONTRACT .
  DATA: LS_CONTRACT TYPE Y03S24999_CNTRCT.

  SELECT SINGLE *
    FROM Y03S24999_CNTRCT
    WHERE ID = @GS_CONTRACT_DETAIL-ID
    INTO CORRESPONDING FIELDS OF @LS_CONTRACT.

  LS_CONTRACT-STATUS = 'Cancelled'.
  LS_CONTRACT-UPDATED_AT = SY-UZEIT + ( 3600 * 5 ).
  LS_CONTRACT-UPDATED_ON = SY-DATUM.

  UPDATE Y03S24999_CNTRCT FROM LS_CONTRACT.

  IF SY-SUBRC <> 0.
    MESSAGE E000(Z03S24999_DOMUS_MSGS) WITH 'Cancel failed!'.
  ELSE.
    PERFORM PREPARE_CONTRACT_DETAIL USING GS_CONTRACT_DETAIL-ID.
    MESSAGE S000(Z03S24999_DOMUS_MSGS) WITH 'Cancel successfully!'.
  ENDIF.
ENDFORM.
*&---------------------------------------------------------------------*
*& Form SIGN_CONTRACT
...
FORM SIGN_CONTRACT .
  ...
  LS_CONTRACT-STATUS = 'Signed'.
  LS_CONTRACT-SIGNATURE =
'https://firebasestorage.googleapis.com/v0/b/sap-domus.appspot.com/o/images%2FHuySignature.png?alt=media&token=122341c9-9728-456d-8e0b-419cb043d87e'.
  LS_CONTRACT-SIGNED_AT = SY-UZEIT + ( 3600 * 5 ).
  LS_CONTRACT-SIGNED_ON = SY-DATUM.

  UPDATE Y03S24999_CNTRCT FROM LS_CONTRACT.

  IF SY-SUBRC <> 0.
    MESSAGE E000(Z03S24999_DOMUS_MSGS) WITH 'Sign failed!'.
  ELSE.
    PERFORM PREPARE_CONTRACT_DETAIL USING GS_CONTRACT_DETAIL-ID.
    MESSAGE S000(Z03S24999_DOMUS_MSGS) WITH 'Sign successfully!'.
  ENDIF.
ENDFORM.
